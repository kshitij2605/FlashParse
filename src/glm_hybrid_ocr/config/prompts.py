...
CLASSIFY_AND_CAPTION_PROMPT = """2枚の画像が提供されています。1枚目はページ全体、2枚目が対象画像です。

まず2枚目の画像を以下の4カテゴリから1つに分類し、次にそのカテゴリに応じたキャプションを生成してください。

カテゴリ:
- chart: 棒グラフ、折れ線グラフ、円グラフ、散布図など数値データを示すグラフ
- figure: フローチャート、組織図、プロセス図、概念図などのダイアグラム
- scanned_text: テキストのみで構成された画像（印刷・手書き文書）
- miscellaneous: 写真、ロゴ、装飾画像、スクリーンショットなどその他の画像

出力形式（厳守）:
[category]カテゴリ名[/category]
[caption]キャプション内容[/caption]

キャプション作成ルール:
- chartの場合：グラフの種類・タイトル・軸・凡例・すべての数値データ・傾向を段落形式で詳述
- figureの場合：図の種類・目的・すべてのテキスト要素・要素間の関係性・流れを段落形式で詳述
- scanned_textの場合：画像内のすべてのテキストを構造を維持して正確に書き起こし
- miscellaneousの場合：画像の種類・主題・重要なテキストやラベル・関連する見出しを1〜3文で簡潔に記述

厳守事項:
- 出力は上記の[category]と[caption]タグのみ
- 「この画像は〜」などの前置き不要
- 日本語のみで回答"""

CLASSIFY_AND_CAPTION_SINGLE_IMAGE_PROMPT = """この画像に含まれるテキストをそのまま抽出してください。

まず画像を以下の4カテゴリから1つに分類してください:
- chart: 数値データを示すグラフ
- figure: ダイアグラム・概念図
- scanned_text: テキストのみの画像
- miscellaneous: 写真・ロゴ・その他
...
キャプション作成ルール:
- すべてのテキストを正確に書き起こすこと
- 元の構造・レイアウトを維持（段落、箇条書き、見出し）
- 表がある場合は行と列の構造を維持
- 読み取れない文字は「[判読不能]」と記載

厳守事項:
- 出力は[category]と[caption]タグのみ
- テキスト以外の説明不要
- 日本語のみ"""
